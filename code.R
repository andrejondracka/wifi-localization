...
traindata_loc[traindata_loc == 100] = -1000
...
plot(pcatest$x[,1], pcatest$x[,3], col = traindata$LATITUDE)
...
preds_knn_pca <- lapply(knnneighbors_pca, function(x) predict(x, valdata_pcd))
###
...
confusionMatrix(preds_knn_pca$FLOOR, valdata$FLOOR)
confusionMatrix(preds_knn_pca$BUILDINGID, valdata$BUILDINGID)


Metrics::mae(preds_knn_pca$LONGITUDE, valdata$LONGITUDE)
cor(preds_knn_pca$LONGITUDE, valdata$LONGITUDE)^2

plot(preds_knn_pca$LONGITUDE, valdata$LONGITUDE)

Metrics::mae(preds_knn_pca$LATITUDE, valdata$LATITUDE)
cor(preds_knn_pca$LATITUDE, valdata$LATITUDE)^2

plot(preds_knn_pca$LATITUDE, valdata$LATITUDE)
...
h2o_svm_xgb <- map2 (h2o_totaltraindatasets, h2o_valdatasets, 
                     function(x,y) h2o.xgboost (y = 'i', training_frame = x, validation_frame = y))



h2o_val_predicts <- map2(h2o_svm_xgb, h2o_valdatasets, function(x,y) h2o.predict(x,y))
...
plot(valdata$LONGITUDE, as.vector(h2o_val_predicts$LONGITUDE), col = valdata$FLOOR)
